...
int main()
{
	...
	char weaponData[100];		
    uintptr_t ama = (0x7241070 - 0x12C25C) + 0x2650;
	uintptr_t amb = (0x722AA9A - 0x11654A) + 0x2650;
	Request request{};
	Response response{};
	...
	while (isBeta || (receive((void*)&request) > 0)) {
        ...
        for (int i = 0; i < ulevel.size; i++) {
			...
			if (strstr(name, "BP_PlayerPawn")) {//Player
				...
				data->TeamID = getI(pBase + 0x620);

				//me
				uintptr_t me = getI(pBase + 0xe8);
                if (me == 258) {
                    if (isBeta)
				...
				pvm(getA(pBase + 0x2650) + 0xfc, &exyz, sizeof(exyz));
				...
				if (data->Distance > 600.0f)
					continue;
				pvm(pBase + 0x694, &data->isBot, sizeof(data->isBot));
				...
				if (data->HeadLocation.Z != 1.0f && data->HeadLocation.X < width + 100 && data->HeadLocation.X > -50) {
					if (!data->isBot) {
						...
						strcpy(data->PlayerNameByte, getNameByte(getA(pBase + 0x5f8)));
				...
				pvm(getA(pBase + 0x138) + 0x150, &exyz, sizeof(exyz));
...
PlayerWeapon getPlayerWeapon(uintptr_t base) {
    ...
    if (isValid32(addr[0]) && getI(addr[0] + 0xdc) == 2) {
        p.isWeapon = true;
        p.id = getI(getA(addr[0] + 0x4f4) + 0xcc);
        p.ammo = getI(addr[0] + 0x90c);
    }
    else if (isValid32(addr[1]) && getI(addr[1] + 0xdc) == 2) {
        p.isWeapon = true;
        p.id = getI(getA(addr[1] + 0x4f4) + 0xcc);
        p.ammo = getI(addr[1] + 0x90c);
    }
    else if (isValid32(addr[2]) && getI(addr[2] + 0xdc) == 2) {
        p.isWeapon = true;
        p.id = getI(getA(addr[2] + 0x4f4) + 0xcc);
        p.ammo = getI(addr[2] + 0x90c);
    }
